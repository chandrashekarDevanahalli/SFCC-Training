...
const cachePaypalCreditFinancingOptionsOcapi = require('dw/system/CacheMgr').getCache('paypalCreditFinancingOptionsOcapi');

const {
    getCalculatedFinancingOptions
} = require('*/cartridge/scripts/paypal/financialApi');
...
creditFinancialOptionsHelper.getAllOptions = function (cost, currencyCode, countryCode, creditType) {
    ...
    var result = cachePaypalCreditFinancingOptionsOcapi.get(sessionCacheKey);
    if (result) {
        return result;
    }
    result = [];
    ...
    cachePaypalCreditFinancingOptionsOcapi.put(sessionCacheKey, result);
    return result;
};
