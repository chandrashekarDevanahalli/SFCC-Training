...
function getOrders(orderNo) {
    var ORDER_NUMBER_PAYPAL_MEHTOD_QUERY = 'orderNo LIKE {0} AND custom.paypalPaymentMethod = \'express\' AND status != {1}';
    var systemOrders = SystemObjectMgr.querySystemObjects('Order', ORDER_NUMBER_PAYPAL_MEHTOD_QUERY, 'creationDate desc', orderNo, Order.ORDER_STATUS_FAILED);
    ...
    while (systemOrders.hasNext()) {
        ...
        if (!paymentInstrumentHelper.isPaypalPaymentInstrument(order)) {
...
OrderMgrModel.prototype.getOrderByTransactionId = function (transactionId) {
    var orderNo;
    var PAYPAL_METHOD_TRANSACTION_QUERY = 'custom.paypalPaymentMethod = \'express\' AND custom.PP_API_TransactionID LIKE {0} AND status != {1}';
    var systemOrder = SystemObjectMgr.querySystemObjects('Order', PAYPAL_METHOD_TRANSACTION_QUERY, 'creationDate desc', transactionId, Order.ORDER_STATUS_FAILED);
