...
const { createErrorLog } = require('*/cartridge/scripts/paypal/paypalUtils');
...
whBase.prototype.verifyWhSignature = function (whEvent, headers, webHookId) {
    ...
    const result = service.setThrowOnError().call(requestData);

    if (!result.ok) {
        const errorObject = JSON.parse(result.errorMessage);
        const error = errorObject.error_description;
...
whBase.prototype.logEventError = function (eventType, endpointName) {
    throw String(endpointName + ':' + Resource.msgf('paypal.webhook.event.does.not.match.error', 'paypalerrors', null, eventType));
};
