...
function validateShipment(shipmentInfo, shipment) {
    ...
    } else if ((shippingAddressFromRequest && !ValidationHelper.validateOrderAddress(shippingAddressFromRequest)) ||
        (shippingMethodIdFromRequest && !ValidationHelper.validateRequestStringValue(shippingMethodIdFromRequest))) {
        resultObject.valid = false;
...
function beforePATCH(_, shipment, shipmentInfo) {
    ...
    if (!validateShipmentResult.valid) {
        Logger.error(validateShipmentResult.message);
