...
function Authorize(orderNumber, paymentInstrument, paymentProcessor) {
    ...
    var error = false;
    try {
        var merkleValidate = require('*/cartridge/scripts/hooks/payment/merkleCreditValidation')

        var merkleCredit = merkleValidate.merkleCreditValidate(paymentInstrument);

        if (merkleCredit) {
            throw new Error(merkleCredit);
        }
        Transaction.wrap(function () {
        ...
        error = true;
        var errorMessage = e.message.indexOf('error') > -1 ? e.message : 'error.technical';
        serverErrors.push(
            Resource.msg(errorMessage, 'checkout', null)
        );
    }
