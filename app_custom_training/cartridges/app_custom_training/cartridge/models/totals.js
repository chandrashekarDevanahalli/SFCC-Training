...
function getRoundoff(lineItemContainer) {
    var Currency = require('dw/util/Currency');
    var Site = require('dw/system/Site');
    var Transaction = require('dw/system/Transaction');

    var roundoff = {};
    var isRoundoffRequired = false;
    var isCartContainsDonationProduct = false;
    var total = lineItemContainer.totalGrossPrice;

    var donationProductLineItem = collections.find(lineItemContainer.getProductLineItems(), function (item) {
        return item.productID === 'donationProductID'
    });

    if (donationProductLineItem) {
        isCartContainsDonationProduct = true;
        var donationLineItemAmount = donationProductLineItem.getAdjustedPrice();
        total = total.subtract(donationLineItemAmount);
    }

    total = total.toNumberString();
    var currencyCode = Site.getCurrent().getDefaultCurrency();
    var currencySymbol = Currency.getCurrency(currencyCode).getSymbol();

    var integerValue = parseInt(total.split(".")[0]);
    var decimalValue = parseInt(total.split(".")[1]);
    var roundoffAmount = 0;
    if (decimalValue > 0) {
        isRoundoffRequired = true;
        integerValue = integerValue + 1;
        roundoffAmount = (integerValue - total).toFixed(2);
    }

    roundoff.isRoundoffRequired = isRoundoffRequired;
    roundoff.isCartContainsDonationProduct = isCartContainsDonationProduct;
    roundoff.roundoffAmount = roundoffAmount;
    roundoff.roundOffAmountWithCurrency = currencySymbol + roundoffAmount;
    ...
    collections.forEach(lineItemContainer.couponLineItems, function (couponLineItem) {
        var priceAdjustments = collections.map(
            ...
            function (priceAdjustment) {
                return {
                    callOutMsg: (typeof priceAdjustment.promotion !== 'undefined' && priceAdjustment.promotion !== null) ? priceAdjustment.promotion.calloutMsg : ''
                };
            });
...
function totals(lineItemContainer) {
    if (lineItemContainer) {
        ...
        this.roundoff = getRoundoff(lineItemContainer);
